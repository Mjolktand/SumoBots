...
void linesens_read(){
  ...
  while (!(TIFR0 & (1 << OCF0A))){
    if (!(PINB & (1 << PINB0)) && sensorValues[0] == 0){
        sensorValues[0] = TCNT0;
      }
     if (!(PINB & (1 << PINB1)) && sensorValues[1] == 0){
         sensorValues[1] = TCNT0;
       }
     if (!(PIND & (1 << PIND4)) && sensorValues[2] == 0){
         sensorValues[2] = TCNT0;
       }
     if (!(PIND & (1 << PIND5)) && sensorValues[3] == 0){
         sensorValues[3] = TCNT0;
       }
     if (!(PIND & (1 << PIND6)) && sensorValues[4] == 0){
         sensorValues[4] = TCNT0;
       }
     if (!(PIND & (1 << PIND7)) && sensorValues[5] == 0){
